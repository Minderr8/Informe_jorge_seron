...
#define LINEAL_VECTOR_RANDOM_FROM_VECTOR_BINARY_SEARCH_COUNT                   \
...
#define NORMAL_VECTOR_RANDOM_BINARY_SEARCH_COUNT                               \
  LINEAL_VECTOR_RANDOM_BINARY_SEARCH_COUNT
#define NORMAL_VECTOR_RANDOM_FROM_VECTOR_BINARY_SEARCH_COUNT                   \
  LINEAL_VECTOR_RANDOM_BINARY_SEARCH_COUNT

void writeHeader(std::ofstream &salida) {
  salida << "gen_time_ms,sort_time_ms,true_search_time_ms,true_search_found,"
            "true_search_not_found,select_search_time_ms,select_search_found,"
            "select_search_not_found,";
  salida
      << "gc_gen_time,gc_word_size,gc_total_bits,gc_sample_size_bits,"
      << "gc_true_search_time_ms,gc_true_search_found,gc_true_search_not_found,"
      << "gc_select_search_time_ms,gc_select_search_found,gc_select_search_not_"
         "found,";
  salida << "sf_gen_time_ms,sf_search_time_ms,space_bits_compressed,space_bits_"
            "explicit";
}

template <typename T>
void benchmark(std::ofstream &salida, std::vector<T> &v,
               std::string vector_name, std::uint64_t vector_size_MiB) {

  std::print("    CASE 1: (2/4): sort: {} distribution vector...", vector_name);
  std::fflush(stdout);
  ...
  std::print("    CASE 1: (3/4): random binary search ({}n): {} distribution "
             "vector...",
             LINEAL_VECTOR_RANDOM_BINARY_SEARCH_COUNT, vector_name);
  std::fflush(stdout);
  auto v_true_results =
      bin_search::trueRandom(v, LINEAL_VECTOR_RANDOM_BINARY_SEARCH_COUNT);
  std::println(" DONE.");

  std::print("    CASE 1: (4/4): random from vector binary search ({}n): {} "
             "distribution vector...",
             LINEAL_VECTOR_RANDOM_FROM_VECTOR_BINARY_SEARCH_COUNT, vector_name);
  std::fflush(stdout);
  auto v_select_results = bin_search::selectRandom(
      v, LINEAL_VECTOR_RANDOM_FROM_VECTOR_BINARY_SEARCH_COUNT);
  std::println(" DONE.");
  ...
  std::print("    CASE 2: (1/3): generating gap_coding array from: {} "
             "distribution vector...",
             vector_name);
  std::fflush(stdout);
  ...
  auto v_gap_coding_gen_time =
      std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
  std::println(" DONE.");

  std::print("    CASE 2: (2/3): random binary search: {} gap_coding...",
             vector_name);
  std::fflush(stdout);
  auto v_gap_coding_true_results = bin_search::trueRandom(
      v_gap_coding, LINEAL_VECTOR_RANDOM_FROM_VECTOR_BINARY_SEARCH_COUNT);
  std::println(" DONE.");

  std::print("    CASE 2: (3/3): random from gap_coding binary search: {} "
             "gap_coding...",
             vector_name);
  std::fflush(stdout);
  auto v_gap_coding_select_results = bin_search::selectRandom(
      v_gap_coding, LINEAL_VECTOR_RANDOM_FROM_VECTOR_BINARY_SEARCH_COUNT);
  std::println(" DONE.");
  ...
  std::print("    CASE 3: (1/2): build Shannon-Fano: {} distribution vector...",
             vector_name);
  std::fflush(stdout);
  t0 = std::chrono::high_resolution_clock::now();
  ...
  t1 = std::chrono::high_resolution_clock::now();
  std::println(" DONE.");
  auto sf_v_build_time =
      std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();

  std::print("    CASE 3: (2/2): search ({}n): {} distribution vector...",
             LINEAL_VECTOR_RANDOM_FROM_VECTOR_BINARY_SEARCH_COUNT, vector_name);
  std::fflush(stdout);
  auto sf_v_results = binary_search_sf::sfSelectRandom(
      sf_v, v, LINEAL_VECTOR_RANDOM_FROM_VECTOR_BINARY_SEARCH_COUNT);
  std::println(" DONE.");
...
int main(int argc, char **argv) {
  bool es_benchmark = (argc == 2 && strncmp(argv[1], "--benchmark", 11) == 0) ||
                      (argc == 4 && strncmp(argv[1], "--benchmark", 11) == 0 &&
                       strncmp(argv[2], "-o", 2) == 0);
  ...
  if (es_benchmark) {
    ...
    salida << "SETTINGS" << "\n";
    salida << "RANDOM_BINARY_SEARCH_COUNT,"
           << LINEAL_VECTOR_RANDOM_BINARY_SEARCH_COUNT << "\n\n";
    ...
    writeHeader(salida);
    salida << "\n";
    // ~10^6, ~10^7, ~10^8 elementos (8 bytes/elem). Ajustar segun RAM y tiempo
    // disponibles. std::vector<std::uint64_t> VECTOR_SIZES_MEBIBYTE = {8, 76,
    // 763};
    std::vector<std::uint64_t> VECTOR_SIZES_MEBIBYTE = {
    ...
    for (auto &vector_size_MiB : VECTOR_SIZES_MEBIBYTE) {
      ...
      std::print("CASE 1: (1/4): generate: lineal distribution vector ({} "
                 "mebibytes)...",
                 vector_size_MiB);
      std::fflush(stdout);

      auto t0 = std::chrono::high_resolution_clock::now();
      auto lineal_vector = vec_gen::linealVector<int64_t>(
          vector_size_MiB * LINEAL_VECTOR_SIZE, EPSILON);
      auto t1 = std::chrono::high_resolution_clock::now();
      std::println(" DONE.");
      auto lineal_gen_time =
    ...
    // std::vector<std::float64_t> STANDARD_DEVIATIONS = {1, 2, 3, 4, 5, 6, 7,
    ...
    for (auto &vector_size_MiB : VECTOR_SIZES_MEBIBYTE) {
      for (auto &standard_deviation : STANDARD_DEVIATIONS) {
        ...
        std::print("CASE 1: (1/4): generate: normal distribution vector ({} "
                   "mebibytes, {} sigma)...",
                   vector_size_MiB, standard_deviation);
        std::fflush(stdout);

        auto t0 = std::chrono::high_resolution_clock::now();
        auto lineal_vector = vec_gen::normalVector<int64_t>(
            vector_size_MiB * LINEAL_VECTOR_SIZE, standard_deviation);
        auto t1 = std::chrono::high_resolution_clock::now();
        std::println(" DONE.");
        auto lineal_gen_time =
            std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0)
  ...
  } else if ((argc == 5 && strncmp(argv[1], "-i", 2) == 0 &&
              strncmp(argv[3], "-c", 2) == 0) ||
    ...
    auto buscar_e_imprimir = [](std::int64_t x, std::int64_t pos,
                                long long ns) {
      if (pos >= 0)
    ...
    if (caso == 1) {
      std::println("Caso 1 (busqueda binaria explicita). {} elementos. Ingrese "
                   "valores, 'q' para salir:",
                   datos.size());
      while (std::getline(std::cin, linea)) {
        ...
        std::int64_t pos = (it != datos.end() && *it == x)
                               ? (std::int64_t)(it - datos.begin())
                               : -1;
      ...
      std::print("Construyendo Gap Coding...");
      std::fflush(stdout);
      gap_coding::GapArray<std::int64_t> estructura(datos);
      std::println(" listo. {} elementos. Ingrese valores, 'q' para salir:",
                   datos.size());
      while (std::getline(std::cin, linea)) {
        if (linea == "q" || linea == "Q")
          break;
      ...
      std::print("Construyendo Shannon-Fano...");
      std::fflush(stdout);
    ...
    std::println("             ./main --benchmark -o <archivo_salida.csv>   "
                 "(salida.csv por defecto)");
    std::println("  Archivo:   ./main -i <archivo.csv>");
    std::println("             ./main -i <archivo.csv> -c <caso>");
    std::println("             donde <caso> es 1 (explicito), 2 (gap coding) o "
                 "3 (shannon-fano)");
    std::println("Numeros aceptados: int64_t [-9223372036854775808, "
                 "9223372036854775807] (strtoll)");
    return 1;
  }
